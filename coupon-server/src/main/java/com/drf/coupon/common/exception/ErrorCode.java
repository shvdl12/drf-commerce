...
public enum ErrorCode implements ErrorCodeSpec {
    ...
    SCOPE_AMOUNT_REQUIRED(HttpStatus.BAD_REQUEST, "적용 범위가 지정된 쿠폰은 대상 합계 금액이 필요합니다."),
    INVALID_VALID_DATE_RANGE(HttpStatus.BAD_REQUEST, "쿠폰 유효기간 종료일은 시작일 이후여야 합니다."),
    SCOPE_TARGET_REQUIRED(HttpStatus.BAD_REQUEST, "적용 범위가 지정된 쿠폰은 적용 대상을 지정해야 합니다."),
